...
LyteBox.prototype.start = function(imageLink, doSlide, doFrame) {
	...
	if (this.isLyteframe) {
		...
		if ((imageLink.getAttribute('rel') == 'lyteframe')) { // add a single frame page to frameArray
			...
			this.frameArray.push(new Array(imageLink.getAttribute('href'), imageLink.getAttribute('title'), (rev == null || rev == '' ? 'width: 90%; height: 90%; scrolling: auto;' : rev)));
			...
			if (imageLink.getAttribute('rel').indexOf('lyteframe') != -1) {
				for (var i = 0; i < anchors.length; i++) {
					...
					if (anchor.getAttribute('href') && (anchor.getAttribute('rel') == imageLink.getAttribute('rel'))) {
						...
						this.frameArray.push(new Array(anchor.getAttribute('href'), anchor.getAttribute('title'), (rev == null || rev == '' ? 'width: 90%; height: 90%; scrolling: auto;' : rev)));
					}
				}
